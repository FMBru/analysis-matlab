function out = process_signal_minuit(x,y,options,shouldSave,store_folder,eventID) %process_signal_minuit(x,y,options,run,shouldSave,store_folder,eventID) 
% function for processing the waveform to identify timing parameters
   ...
   % sig.blavg = mean(y(2:20));
    ...
    if(options.en_plot || shouldSave)
        scatter(sig.e_peak_end.x, sig.e_peak_end.y,'blue');
        title('RAW signal');
        ylabel('Voltage, V');
        xlabel('Time, ns');
        grid on;
