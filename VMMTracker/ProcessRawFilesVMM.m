...
trackerVMM.en = 1; %match eventIDs to tracking data and add XY to output
...
%May22: %1: miniCactus, 2: MM3 (VacChamberPos), 3: MM1(Multipad), 4: MM2 (on support plate), 5: MM4 (electron setup)
    ...
    tracker.dutIndex = 5; %DUT index: 1: MM1 (Multipad), 2: MM2(VacChamber), 3: MM3 (ElectronsetupMM), 4: MM4 (Picolarge main tracker), 5:MM5 (SiPM)
    numberFilesToAnalyse = 0; %max number of files to analyse, 0 -> analyse all in folder
    runInfoString = 'Run 395 Timing MM5 275-455 Pool2 CH2 with VMM + APV - useVMMTracker';
    tracker.dutIndex = 5
    trackerVMM.dutIndex = tracker.dutIndex;
...
trackerVMM.path = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\tracker\reconstructed\asciiRun' run.id '-VMM272.dat'];
...
dutColArray = [1 7 8; 2 10 11; 3 13 14; 4 16 17; 5 22 23; 6 25 26; 7 19 20]; %[dID colXID colYID; ] 
...
if tracker.en
    ...
    trackerFileVMM = fopen(trackerVMM.path,'rt');
    DVMM = textscan(trackerFileVMM, '%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f', 'Delimiter','\t', 'HeaderLines',2, 'CollectOutput',1);
    trackerVMM.data = cell2mat(DVMM);
...
if useVMMTracker
    opts_TR.ch_name = ['C4' run.lecroy_name];
end
...
for (ff=1:run.nfiles)
    ...
    if filesExists==1
        ...
        if tracker.en
                ...
                if useVMMTracker %invert if VMM trigger
                    ch_tr.y(:,m) = -ch_tr.y(:,m)-0.7;
                end
                
                event_id = process_tr_bitstream(t_vec_mcp, ch_tr.y(:,m), opts_TR);
                if shouldPlotTrackerWaveform
                    close all
                    plot(t_vec_mcp, ch_tr.y(:,m));
                    pause(0.5);
                end
...
%             end
            ...
            if(minuit==1)
                MM_temp = process_signal_minuit(t_vec_mm-etime,ch_mm.y(:,m),opts_MM,shouldSave,storeFolderSignalDUT,eventIDTracker);
            else
            ...
            if(minuit==1)
                MCP_temp = process_signal_minuit(t_vec_mcp-etime,ch_mcp.y(:,m),opts_MCP,shouldSave,storeFolderSignalRef,eventIDTracker);
            else
