...
addpath '..\..\Picosec\CommonFunctions';
addpath '..\..\Picosec\CommonFunctions\SCP';
addpath '..\..\Picosec\SAMPIC';
%addpath '..\..\SPEAnalysisPolyaMeanFinalVersion';
addpath '..\Analysis\Functions';

fid = fopen('batchAnalyseList1.txt');
...
while  length(analyseEntries{1})>1
    ...
    if strcmp(oscilloscopeString,'SAMPIC')
        ...
        %ProcessRawFiles
        Light_Parallel_ProcessRawFiles1
    end
    ...
    fid = fopen('batchAnalyseList1.txt', 'r') ;              % Open source file.
    line1 = fgetl(fid) ;                                  % Read/discard line.
    lineDiscard = fgetl(fid) ;                                  % Read/discard line.
    buffer = fread(fid, Inf) ;                    % Read rest of the file.
    fclose(fid)
    
    fid = fopen('batchAnalyseList1.txt', 'w')  ;   % Open destination file.
