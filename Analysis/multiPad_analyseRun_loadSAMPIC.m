...
%% loading data
%loadVariableFromFiles_Multipad

allPadsTable = table(padIDArray (:), eventIDArray(:), trackerX(:), trackerY(:), MCP_maxy(:), MM_maxy(:), MM_maxy(:), time_diff_sigmoid(:), MM_riseTime(:), 'VariableNames', {'padID','eventID','X', 'Y', 'MCP_amp', 'MM_amp', 'e_peak_MM', 'SAT', 'riseTime'});
...
padArray = [12, 13, 14, 22, 23, 24, 32, 33, 34];        %numbers of the analysed pads
...
%     load(fileList{i}); 
    singlepad_tables{i} = allPadsTable(allPadsTable.padID == padArray(i), :);
    singlepad_tables{i}.padID = [];
end

clearvars -except twalk geom singlepad_tables padArray store_folder runIDsToAnalyse channelsEnabled channelToAnalyse run MM_data MCP_data time_diff time_diff_sigmoid MCP_maxy MM_maxy trackerX trackerY eventIDArray padIDArray MM_epeak MCP_epeak MM_riseTime
...
for i=1:length(padArray)
    ...
    timewalk_folder_corr = [store_folder '\timewalk\pad' num2str(padArray(i)) '\Correction-FULL'];
    mkdir(timewalk_folder_corr);

    SATCorrvars{i} = [SATvars{i} '_Corr'];
    finalTable.(SATCorrvars{i}) = finalTable.(SATvars{i}) - (twalk_fn_minuit(pad.TimeWalkCorr_full{i}, (finalTable.(e_peakvars{i}))'))';
...
time_folder = [store_folder '\timeResolution\WEIGHTED_E_PEAK_CORRECTED_FULL'];
mkdir(time_folder);
