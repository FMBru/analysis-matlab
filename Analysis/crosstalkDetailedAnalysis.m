...
for i=1:length(padArray)
    ...
    pad_window_left = pad_x_center - 0.9*full_geom.pad_window_length/2;
    pad_window_right = pad_x_center + 0.9*full_geom.pad_window_length/2;
    pad_window_up = pad_y_center + 0.9*full_geom.pad_window_length/2;
    pad_window_bottom = pad_y_center - 0.9*full_geom.pad_window_length/2;
    ...
    for j=1:length(padExcluded)
        ...
        distribution(cross_high_table.(['SAT' padCrosstalk]),100,['SAT-pad' padCrosstalk],'ns', pad_folder, ['-geoselecting-pad' padSelected], true); %SAT distribution (high Amp)
        distribution(cross_high_table.(['MM_amp' padCrosstalk]),100,['Amplitude-pad' padCrosstalk],'V', pad_folder, ['-geoselecting-pad' padSelected], false); %amplitude distribution (high Amp)
        distribution(cross_high_table.(['riseTime' padCrosstalk]),100,['riseTime-pad' padCrosstalk],'V', pad_folder, ['-geoselecting-pad' padSelected],false); %riseTime distribution (high Amp)

        correlationPlots(cross_high_table.(['SAT' padCrosstalk]), ratio_high, ['SAT-pad' padCrosstalk], 'ns', ['Ratio-pad' padSelected '-by-pad' padCrosstalk], 'adim', pad_folder, ['-geoselecting-pad' padSelected], true, false);
        correlationPlots(cross_high_table.(['SAT' padCrosstalk]), cross_high_table.X, ['SAT-pad' padCrosstalk], 'ns', ['X-coordinate-on-pad' padSelected], 'mm', pad_folder, ['-geoselecting-pad' padSelected], true, false);
        correlationPlots(cross_high_table.(['SAT' padCrosstalk]), cross_high_table.Y, ['SAT-pad' padCrosstalk], 'ns', ['Y-coordinate-on-pad' padSelected], 'mm', pad_folder, ['-geoselecting-pad' padSelected], true, false);
