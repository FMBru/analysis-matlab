...
if exist('batchProcess','var') == 1
    ...
    runInfoString = 'Multipad old pad 38 - comparing';
end

enDebugPlots = false;
correctFilenumberOffset = false;
% 
% if strcmp(run.oscilloscope, 'Pool2') && strcmp(opts_MM.chID, '2') && str2num(run.id) > 214 && str2num(run.id) < 218
...
%%  to check crosstalk waveforms (don't use parallel process)
% load('\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\eventIDcrosstalk.mat');
% load('\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\eventIDcrosstalk_low.mat');
% storeFolderSignalRef_high = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_high\REF'];
% mkdir(storeFolderSignalRef_high);
% storeFolderSignalRef_low = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_low\REF'];
% mkdir(storeFolderSignalRef_low);
% storeFolderSignalDUT_high = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_high\DUT'];
% mkdir(storeFolderSignalDUT_high);
% storeFolderSignalDUT_low = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_low\DUT'];
% mkdir(storeFolderSignalDUT_low);
% storeFolderSignalRef = storeFolderSignalRef_high;
% storeFolderSignalDUT = storeFolderSignalDUT_high;
...
% MM_data_to_save_high = cell(length(eventIDs_cutting_on_pad34_amplitude),1);
% MM_data_to_save_low = cell(length(eventIDs_cutting_on_pad34_amplitude_low),1);
% MCP_data_to_save_high = cell(length(eventIDs_cutting_on_pad34_amplitude),1);
% MCP_data_to_save_low = cell(length(eventIDs_cutting_on_pad34_amplitude_low),1);
...
%     end
    ...
    if filesExists==1
        ...
        %for m=1:nTRCseg
            ...
            %to check crosstalk waveforms
%             if ismember(eventIDTracker, eventIDs_cutting_on_pad34_amplitude)
%                 shouldSave = 1;
%                 storeFolderSignalDUT = storeFolderSignalDUT_high;
%                 storeFolderSignalRef = storeFolderSignalRef_high;
%             elseif ismember(eventIDTracker, eventIDs_cutting_on_pad34_amplitude_low)
%                 shouldSave = 1;
%                 storeFolderSignalDUT = storeFolderSignalDUT_low;
%                 storeFolderSignalRef = storeFolderSignalRef_low;
%             else
%                 shouldSave = 0;
%             end
            ...
            %to check crosstalk waveforms
%             if ismember(eventIDTracker, eventIDs_cutting_on_pad34_amplitude)
%                 idx = find(eventIDs_cutting_on_pad34_amplitude == eventIDTracker);
%                 MM_data_to_save_high{idx} = MM_temp;
%                 MCP_data_to_save_high{idx} = MCP_temp;
%             elseif ismember(eventIDTracker, eventIDs_cutting_on_pad34_amplitude_low)
%                 idx = find(eventIDs_cutting_on_pad34_amplitude_low == eventIDTracker);
%                 MM_data_to_save_low{idx} = MM_temp;
%                 MCP_data_to_save_low{idx} = MCP_temp;
%             else
%                 shouldSave = 0;
%             end
...
% to check crosstalk waveforms
% save('\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_high\struct_of_MM_and_MCP_high.mat', 'MM_data_to_save_high', 'MCP_data_to_save_high');
% save('\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\Multipad\Crosstalk_low\struct_of_MM_and_MCP_low.mat', 'MM_data_to_save_low', 'MCP_data_to_save_low');
%% save scope settings
