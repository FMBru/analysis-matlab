...
store_folder = ['Martaanalyzed/Run' run.id '-' run.oscilloscope];


if exist('batchProcess','var') == 1
    store_folder = ['Martaanalyzedanalyzed/Run',run.id,'-',run.oscilloscope,'-ref',MCPnameString,'-dut',DUTnameString];
else
    store_folder = ['Martaanalyzedanalyzed/Run',run.id,'-',run.oscilloscope,'-ref',opts_MCP.ch_name,'-dut',opts_MM.ch_name];
...
%time_avg_raw = median(time_diff_sigmoid);  % assume mu from the median vaule
%time_min = time_avg_raw - 2;     % predicted resolution 100ps cut 3 sigma
%time_max = time_avg_raw + 2;     % left and 3 sigma right from median
...
%only observeMM
%glbl_cut = MM_maxy>0.01*max(MM_maxy)& MM_maxy<0.95*max(MM_maxy) & trackerX~=0 & trackerY~=0;
...
str_title = sprintf('%s:\n SAT map (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
...
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n Time resolution map (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
...
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n Mean error map (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
...
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n DUT MM map (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
movegui(gcf,'west');
...
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n DUT E-peak charge (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
...
h.Label.String = 'Amplitude, V';
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n REF MCP amplitude map (\\phi_{avg} = %2.1f mm)', run.name, 2*area.radius);
title(str_title);
movegui(gcf,'center');
...
str_title = sprintf('%s:\n DUT amplitude cross-sections ',run.name);
title(str_title);
...
if exist('batchProcess','var') == 1
    uploadCommand = append('pscp -r -pw ','Win_Admin',' C:\Users\GDD\Documents\MATLAB\Picosec\Analysis\Martaanalyzed\Run',run.id,'-',run.oscilloscope,'-ref',MCPnameString,'-dut',DUTnameString,' ','gdd','@lxplus.cern.ch:/eos/project/p/picosec/testbeam/2022_May_h4/Results');
    uploadResult = system(convertStringsToChars(uploadCommand))
    
else
    uploadCommand = append('pscp -r -pw ','Win_Admin',' C:\Users\GDD\Documents\MATLAB\Picosec\Analysis\Martaanalyzed\Run',run.id,'-',run.oscilloscope,'-ref',opts_MCP.ch_name,'-dut',opts_MM.ch_name,' ','gdd','@lxplus.cern.ch:/eos/project/p/picosec/testbeam/2022_May_h4/Results');
    uploadResult = system(convertStringsToChars(uploadCommand))
