op% MATLAB/OCTAVE implementation of processing code for PICOSEC
% version date: 27 Jan 2025
close all
...
numberDebugPlots = 5;
saveSignalWaveformsNumber = numberDebugPlots;
...
run.year = '2025 January ';
run.name = ['LAB ' run.year ' RUN ' run.id];
run.pathEOS=['D:\asena\measurements\' run.id '\'];
disp(run.pathEOS)
run.nfiles = find_fileNo(run.pathEOS);

tracker.path = ['\\eosproject-smb\eos\project\p\picosec\lab\Marta\Photodetection\Laser\' run.id '\'];
...
run.debugPlotsPath=['\\eosproject-smb\eos\project\p\picosec\lab\Marta\Photodetection\Laser\Results\DebugPlots\Run' run.id '-'  run.oscilloscope '\'];
mkdir(run.debugPlotsPath);

storeFolderSignalRef = ['\\eosproject-smb\eos\project\p\picosec\lab\Marta\Photodetection\Laser\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\REF'];
mkdir(storeFolderSignalRef);
storeFolderSignalDUT = ['\\eosproject-smb\eos\project\p\picosec\lab\Marta\Photodetection\Laser\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\DUT'];
...
%for (ff=file_start:file_stop)
    ...
    if exist(ch_mm_str,'file')==2 && exist(ch_mcp_str,'file')==2
        filesExists=1;
...
%                     grid on;
           ...
           % close all
            ...
            if refSignalValid & dutSignalValid 
