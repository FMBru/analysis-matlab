...
if exist('batchProcess','var') == 1
    ...
    runInfoString = 'Pavia Ne iC4H10 90 10 A285 C450 (re-analysed MM5)';
end
...
%     end
    ...
    if filesExists==1
        ...
        for m=1:nTRCseg
            ...
            yPos = 0;
       	    ...
       	    if enableTracker
                ...
                trackIdx = find(TR_DATA(:,1) == eventIDTracker);
                if trackIdx>0
                    %is valid trackIdx -> get XY
                    xPos = TR_DATA(trackIdx,dutColArray(TR_DUTINDEX,2));
                    yPos = TR_DATA(trackIdx,dutColArray(TR_DUTINDEX,3));
                    if size(xPos,1)>1
                        xPos = xPos(1);
                    end
                    if size(yPos,1)>1
            ...
            if refSignalValid && dutSignalValid && trackerSignalValid
                ...
                local_time_diff = [local_time_diff, MM_temp.cfd.time-MCP_temp.cfd.time];
		        local_time_diff_sigmoid = [local_time_diff_sigmoid, MM_temp.sigmoid.timepoint-MCP_temp.sigmoid.timepoint];

                localMMData = [localMMData; MM_temp]; 
                localMCPData = [localMCPData; MCP_temp];

            else
    		    local_time_diff_sigmoid = [local_time_diff_sigmoid, 0];
		        localMaxY(m) = 0;
                localMaxYMCP(m) = 0;
            end
	    ...
	    time_diff_onlyValid{ff} = local_time_diff; %---> this will go in time_diff
