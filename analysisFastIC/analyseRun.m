...
%dataFilePath = append('/eos/project/p/picosec/testbeam/2023_July_h4/info/MatlabRunInfo');
%dataFilePath = append('/eos/project/p/picosec/testbeam/2023_August_h4/info/MatlabRunInfo');
dataFilePath = append('/eos/project/p/picosec/testbeam/2024_June_h4/info/MatlabRunInfo');
...
%store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_July_h4\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString];
%store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_August_h4\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString];
store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_June_h4\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString];
mkdir(store_folder);
...
for i=1:length(MCP_data)
    ...
    % Following recommendation from Alexandra, try out CFD interpolation (25/July/2023)
    % time_MM(i) = MM_data(i).sigmoid.timepoint;
    time_MM(i) = MM_data(i).cfd.time;
    ...
    ymax_MM(i)=MM_data(i).sig.charge.e_peak;      % ORIGINALLY: take maximum of MM. FASTIC: take width of energy output.
    ymin_MM(i)=MM_data(i).sig.min.y;      % take minimum of MM (NOT USED)
...
%time_avg_raw = mean(time_diff_sigmoid(MM_maxy>0.01));  % assume mu from the median vaule
...
%time_avg_raw = 12.5;
%time_avg_raw;
%time_min = time_avg_raw - 1.2;     % predicted resolution 100ps cut 3 sigma
%time_max = time_avg_raw + 1.2;     % left and 3 sigma right from median
% FOR MULTI-CHANNEL FASTIC ONLY
time_min = -50.0;
time_max = +50.0;
...
% with tracker
% IN ADDITION: remove points for FastIC
% => if the peak amplitude (e_peak_MM) is too low, the energy output is too
%    narrow and also too low (despite being a digitial signal). Then the algorithm in
%    process_signal_minuit_FastIC finds not the energy output, but the timing
%    output, which gives a wrong width of 120 ns instead of max. ~ 50 ns.
%    This 100 ns point needs to be removed, which is done here.
%glbl_cut = time_diff_sigmoid>time_min & time_diff_sigmoid<time_max & ...
%    MCP_maxy>0.01*max(MCP_maxy) & MM_maxy>0.01*max(MM_maxy)& ...
%    MCP_maxy<0.95*max(MCP_maxy) & ...
%    trackerX~=0 & trackerY~=0 & ...
%    e_peak_MM < 100.0;
% CHANGE IN AUGUST 2023: REMOVE MCP SATURATION CUT...
% ADDITIONAL CHANGE: IN PROCESS_SIGNAL_MINUTI_FASTIC, THE FUNCTION TO
% DETERMINE THE WIDTH WAS MODIFIED, SUCH THAT IF THE WRONG WIDHT IS TAKEN,
% THE WIDTH WILL BE CHANGED TO 9999.9 ns. THIS IS THEN REMOVED BY THE 500
% ns CUT. THIS ALLOWS ALSO TO ACCEPT EVENTS, WITH HIGHER AMPLITUDE.
glbl_cut = time_diff_sigmoid>time_min & time_diff_sigmoid<time_max & ...
    MM_maxy>0.01*max(MM_maxy)& ...
    trackerX~=0 & trackerY~=0 & ...
    e_peak_MM < 500.0;

%additional cuts
glbl_cut = glbl_cut & MCP_maxy>0.25;
...
% Remove points for a proper timewalk calibration for FastIC
% => NOT NEEDED ANYMORE SINCE THE GLOBAL CUT WAS MODIFIED ACCORDINGLY
%twalk.e_peak(end-2:end) = [];
%twalk.mean_sat(end-2:end) = [];
%twalk.npts(end-2:end) = [];
%twalk.rms(end-2:end) = [];
%twalk.err(end-2:end) = [];
%twalk.e_peak_err_p(end-2:end) = [];
%twalk.e_peak_err_n(end-2:end) = [];

% Remove NaN values
NaNIndexX = find(isnan(twalk.e_peak));
NaNIndexY = find(isnan(twalk.mean_sat));
twalk.e_peak(NaNIndexX) = [];
twalk.mean_sat(NaNIndexY) = [];
twalk.npts(NaNIndexX) = [];
twalk.rms(NaNIndexX) = [];
twalk.err(NaNIndexX) = [];
twalk.e_peak_err_p(NaNIndexX) = [];
twalk.e_peak_err_n(NaNIndexX) = [];

% fit correction function using Levenberg-Marquardt
fit_data = [];
...
options = optimoptions('lsqcurvefit','Algorithm','levenberg-marquardt','MaxIterations',10000,'MaxFunctionEvaluations',10000);
[p, chi] = lsqcurvefit(@twalkLM, p0, twalk.e_peak, twalk.mean_sat, [], [], options);

%if runInfoString == 'FastIC-MultiChannel-MM1-520C-275A-pad15-hardcoded'
%    p = [1.95362, 4.27822, 0.430469];
%end

%if runInfoString == 'FastIC-MultiChannel-MM1-520C-275A-pad16-hardcoded'
%    p = [11.8593, 119.501, 1.42294];
%end

%if runInfoString == 'FastIC-MultiChannel-MM1-520C-275A-pad25-hardcoded'
%    p = [2.31350, 2.72802, 1.12901];
%end

%if runInfoString == 'FastIC-MultiChannel-MM1-520C-275A-pad26-hardcoded'
%    p = [6.44001, 9.02994, 4.32764];
%end
...
xlabel('Electron peak charge / ns')
ylabel('SAT / ns')
...
writematrix(twalk.e_peak, [store_folder '\Run' run.id '_timewalk_epeak.csv'])
writematrix(twalk.mean_sat, [store_folder '\Run' run.id '_timewalk_sat.csv'])
writematrix(twalk.err, [store_folder '\Run' run.id '_timewalk_err.csv'])
writematrix(twalk.e_peak_err_n, [store_folder '\Run' run.id '_timewalk_e_peak_err_n.csv'])
writematrix(twalk.e_peak_err_p, [store_folder '\Run' run.id '_timewalk_e_peak_err_p.csv'])
writematrix(twalk.p, [store_folder '\Run' run.id '_timewalk_fitresults.csv'])
...
% Check if time walk correction will be actually applied
disp(time_diff_sigmoid);
...
% % PAD 67
% if runInfoString == 'FastIC-440C-275A-multi-channel-ch5'
%     pad.xc=30;
%     pad.yc=40;
% end
% 
% % PAD 68
% if runInfoString == 'FastIC-440C-275A-multi-channel-ch1'
%     pad.xc=20;
%     pad.yc=40;
% end
% 
% % PAD 77
% if runInfoString == 'FastIC-440C-275A-multi-channel-ch0'
%     pad.xc=30;
%     pad.yc=30;
% end
% 
% % PAD 78
% if runInfoString == 'FastIC-440C-275A-multi-channel-ch4'
%     pad.xc=20;
%     pad.yc=30;
% end
%PAD 67

if runInfoString == 'FastIC-440C-275A-multi-channel-run2-ch0'
    pad.xc=19.0;
    pad.yc=30.0;
end

if runInfoString == 'FastIC-440C-275A-multi-channel-run2-ch1'
    pad.xc=9.0;
    pad.yc=30.0;
end

if runInfoString == 'FastIC-440C-275A-multi-channel-run2-ch4'
    pad.xc=9.0;
    pad.yc=40.0;
end

if runInfoString == 'FastIC-440C-275A-multi-channel-run2-ch5'
    pad.xc=19.0;
    pad.yc=40.0;
end
...
dataEff = [numberAllEvents numberEventsWithTrackerActiveArea numberEventsWithTrackerSamplingArea numberAcceptedRefEventsActiveArea numberAcceptedDUTEventsActiveArea efficiencyREFActiveArea efficiencyDUTActiveArea numberAcceptedRefEventsSamplingArea numberAcceptedDUTEventsSamplingArea efficiencyREFSamplingArea efficiencyDUTSamplingArea]
writematrix(dataEff, [store_folder '\Run' run.id '_efficiencyDataWithoutGlblCut.csv'])

%% repeat efficiencyCalculation with cuts on timing and amp as glb cut

% CHANGE IN AUGUST 2023 TO NOT ARTIFICALLY DESCREASE EFFICIENCY BECAUSE OF
% CUT ON MCP AMPLITUDE...
...
%    eventsREFmaxY<0.95*max(eventsREFmaxY)&...
%    eventsDUTmaxY < 100.0;
acceptanceCut = eventsTimeDiffSigmoid>time_min & eventsTimeDiffSigmoid<time_max & ...
...
dataEff = [numberAllEvents numberEventsWithTrackerActiveArea numberEventsWithTrackerSamplingArea numberAcceptedRefEventsActiveArea numberAcceptedDUTEventsActiveArea efficiencyREFActiveAreaAccepted efficiencyDUTActiveAreaAccepted numberAcceptedRefEventsSamplingArea numberAcceptedDUTEventsSamplingArea efficiencyREFSamplingAreaAccepted efficiencyDUTSamplingAreaAccepted]
writematrix(dataEff, [store_folder '\Run' run.id '_efficiencyDataWithGlblCut.csv'])
...
for i=1:length(area.x_vec)
    %dlmwrite([store_folder '\Run' run.id '_sat_' int2str(i) '.csv'], i, 'delimiter', ',');
    %dlmwrite([store_folder '\Run' run.id '_polya_' int2str(i) '.csv'], i, 'delimiter', ',');
    for j=1:length(area.y_vec)
        %dlmwrite([store_folder '\Run' run.id '_sat_' int2str(i) '.csv'], j, 'delimiter', ',', '-append');
        %dlmwrite([store_folder '\Run' run.id '_polya_' int2str(i) '.csv'], j, 'delimiter', ',', '-append');
        ...
        %dlmwrite([store_folder '\Run' run.id '_sat_' int2str(i) '.csv'], time_diff_sigmoid(cut_circ), 'delimiter', ',', '-append');
        area.sat(i,j) = mean(time_diff_sigmoid(cut_circ));
        area.rms(i,j) = std(time_diff_sigmoid(cut_circ));
        %dlmwrite([store_folder '\Run' run.id '_polya_' int2str(i) '.csv'], ymax_MM(cut_circ), 'delimiter', ',', '-append');
...
writematrix(area.xx, [store_folder '\Run' run.id '_efficiencyDUT_X.csv'])
writematrix(area.yy, [store_folder '\Run' run.id '_efficiencyDUT_Y.csv'])
writematrix(area.dutEfficiency, [store_folder '\Run' run.id '_efficiencyDUT_EFF.csv'])
...
writematrix(area.xx, [store_folder '\Run' run.id '_RES_map_X.csv'])
writematrix(area.yy, [store_folder '\Run' run.id '_RES_map_Y.csv'])
writematrix(area.rms*1000, [store_folder '\Run' run.id '_RES_map_res.csv'])
...
writematrix(area.xx, [store_folder '\Run' run.id '_DUT_amp_X.csv'])
writematrix(area.yy, [store_folder '\Run' run.id '_DUT_amp_Y.csv'])
writematrix(area.amp, [store_folder '\Run' run.id '_DUT_amp_amp.csv'])
...
writematrix(MM_e_peak_charge(cut_sampling), [store_folder '\Run' run.id '_echarge_hist-samplingArea.csv'])

%% BG level comparison ONLY SAMPLING AREA
figure
histogram(blavg_MCP(cut_sampling)*1000,100)
hold on
h=histogram(blavg_MM(cut_sampling)*1000,100);
hold on
xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
xlabel('Background level, mV')
ylabel('events');
grid on
xlim([-10 10]);
legend('REF','MM');
title_str = sprintf('%s \n Baseline level REF:\\mu = %4.4f mV, DUT:\\mu = %4.4f mV',runTitleString, 1000*mean(blavg_MCP(cut_sampling)),1000*mean(blavg_MM(cut_sampling)));
title(title_str)
movegui(gcf,'east');
saveas(gcf,[store_folder '\Run' run.id '_bgLevel_comparison-samplingArea.png'])

%% BG RMS comparison ONLY SAMPLING AREA
figure
histogram(blrms_MCP(cut_sampling)*1000,100)
hold on
h=histogram(blrms_MM(cut_sampling)*1000,100);
hold on
xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
xlabel('Background RMS, mV')
ylabel('events');
grid on
legend('REF','MM');
xlim([0 5]);
title_str = sprintf('%s \n Baseline RMS REF:\\mu = %4.4f mV, DUT:\\mu = %4.4f mV',runTitleString, 1000*mean(blrms_MCP(cut_sampling)),1000*mean(blrms_MM(cut_sampling)));
title(title_str)
movegui(gcf,'east');
saveas(gcf,[store_folder '\Run' run.id '_bgRMS_comparison-samplingArea.png'])
...
figure
%rng(abs(pd.mu));
h = histogram(time_diff_sigmoid(cut_sampling),150);
hold on
fit_data = [];
...
% message = [message sprintf('  RMS_{hist} = %2.1f ps ',1000*std(time_diff_sigmoid(cut_sampling)))];
...
writematrix(time_diff_sigmoid(cut_sampling), [store_folder '\Run' run.id '_time_res-samplingArea-singleGauss.csv'])
...
ylabel('y-axis, mm');
grid on
str_title = sprintf('Ttracker data');
title(str_title);
legend('Trigger','Hits','Observation path','DUT center','Circle for resolution measurement');
if length(trackerX(glbl_cut))>0
    xlim([min(trackerX(glbl_cut)) max(trackerX(glbl_cut))]);
...
grid on
xlim([0 5]);
title_str = sprintf('Signal rise time mean = %4.4f ns', mean(riseTime(riseTimeCut&glbl_cut)));
title(title_str)
...
message = sprintf('  RMS_{tot} = %2.1f ps ',1000*std(time_diff_sigmoid(cut_sampling)));
