function out = process_signal_minuit_FastIC50(x,y,options,run,shouldSave,store_folder)
% function for processing the waveform to identify timing parameters

    if(options.invert)
        y =-y; % invert detector signal
    end
    ...
    %% Get the index for the SAT fit of the FastIC output
    threshold = 0.5 * (max(y) - min(y)) + min(y);
    yFastICIdx = (y >= threshold);
    sig.max.idx = find(yFastICIdx, 1, 'first');
    ...
    %% find electron peak point (old code for detector signal)
    %{
    ...
    %% Get the index for the Energy measurement
    yEnergyEndIdx = (y >= threshold);
    chargeWidthEndIdx = find(yEnergyEndIdx, 1, 'last');
    %disp(chargeWidthEndIdx);
    %disp(x(chargeWidthEndIdx));
    yEnergyStartIdx = (y <= threshold);
    tempIdx = chargeWidthEndIdx - 10;
    %disp(tempIdx);
    chargeWidthStartIdx = find(yEnergyStartIdx(1:tempIdx), 1, 'last');
    %disp(chargeWidthStartIdx);
    %disp(x(chargeWidthStartIdx));

    %% New code for charge measurement with FastIC
    if x(chargeWidthStartIdx) > 300
        chargeWidth = x(chargeWidthEndIdx) - x(chargeWidthStartIdx);
        %disp(chargeWidth);
        if(options.en_plot || shouldSave)
            scatter(x,y);
            title('FastIC raw signal');
            xlabel('Time / ns');
            ylabel('Signal mplitude / V');
            grid on;
            hold on;
        end
    else
    ...
    sig.charge.e_peak = chargeWidth; %sig.k_V2C * sum(y(sig.startpoint.idx:sig.e_peak_end.idx));
...
%     end
    ...
    sigmoid.start = sig.startpoint.idx - 25;  % 5 samples in original code
    if (sigmoid.start < 1)
        sigmoid.start = 1;
        out.fail = 5;
        return;
    ...
    sigmoid.time20=0; % STILL CALLED TIME20, BUT ACTUALLY IT IS AT 50 %
    sigmoid.sevals=0;
    ...
    % first go forward
    while(sigmoid.time20 < 0.5*sig.max.y) % CHANGED TO 50% FOR FASTIC
