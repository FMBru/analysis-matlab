function out = process_signal_minuit(x,y,options)

    % added hold off for Picolarge not to have several waveforms in the 1 graph
    if(options.en_plot)
    hold off
    end
    ...
    %% trying to have a baseline at 0 for Picolarge
    
    sig.blavg1 = mean(y(1:100));
    y = y - sig.blavg1 ;
    ...
    if(sig.max.idx > options.t_prerms)
        ...
        if(options.en_plot)
            scatter(x([options.t_dead sig.max.idx]),y([options.t_dead sig.max.idx]));
        end
    else
        out.fail = 1;
        return 
    ...
    if(sigmoid.npoints>150 || sigmoid.npoints<=0)
        out.fail = 7;
        return
    end
   ...
   % end
    ...
    if(options.en_plot) 
        ...
        str_title = sprintf('Sigmoid fit result, chi = %4.4f',sigmoid.chi);
        title(str_title)
        ylabel('Voltage, V');
        xlabel('Time, ns');
        grid on
