...
%for (i=5:9)
%for i=99:99
    run.id = 'C103\A275C540\noise_treshold';
    %run.id = sprintf('0%i', i);
    
    channel.id = 'C1';
    run.year = '2023/04';
    run.name = ['PICOSEC - RUN ' run.id ' - Channel ' channel.id];
    %run.path=['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_April_h4\Pool3\Run' run.id '\'];
    %run.path=['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_April_h4\Pool2\Run' run.id '\'];
    run.path=['\\eosproject-smb\eos\project\p\picosec\lab\Chiara\resistive\' run.id '\'];
    %run.path=['\\eosproject-smb\eos\project\p\picosec\lab\Chiara\slow_preampl\' run.id '\'];
    %run.path=['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_April_h4\Pool3\Run' run.id '\'];
    %run.path=['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_April_h4\GDD\Run' run.id '\'];
   %run.path=['\\eosproject-smb\eos\project\p\picosec\testbeam\2022_October_h4\Pool2\Run' run.id '\'];
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           % store_folderWaveforms = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_April_h4\Results\SingleChannelPhotocathodesStudies\Run' run.id '\Waveforms\'];
...
close all
    ...
    %% Amplitude_noise_analysis_fit_subtract

    %AnalyseRun_PEAnalysis
