...
glbl_cut = mm_max_y>0.00 & mm_max_y<0.99*max(mm_max_y); %remove saturated datapoints
...
% for curve fit to subtract noise function
%n_p0=[];
%n_p0(1) = sum(gauss_vals)*h.BinWidth;   % normalization factor
%n_p0(2) = 1;         
%n_p0(3) = 0.1;  
%cmd='min; ret';
...
% for full data fit with noise
full_fit_data = [];
full_fit_data(1,:)=1:length(full_xbins);
full_fit_data(2,:) = h.Values;

% for fit with only noise
noise_fit_data = [];
noise_fit_data(1,:)=1:length(full_xbins);
noise_fit_data(2,:) = noise_arr;

% for fit with data and noise
full_p0=[];
full_p0(1) = sum(h.Values)*h.BinWidth;   % normalization factor
full_p0(2) = 1;         
full_p0(3) = 0.1;  
cmd='min; ret';
...
% full fit
[full_p, full_err, full_chi] = fminuit('polya_minuit',full_p0,full_fit_data(:,1:end),'-b','-c',cmd);
full_polya_cnt_fit = polya_minuit(full_p,full_fit_data(1,:));
%plot(xbins,noise_polya_cnt_fit,'Linewidth',2);
noise_e_peak_amp.mean = sum(full_polya_cnt_fit.*full_xbins)/sum(full_polya_cnt_fit);
[full_dummy, full_e_peak_amp.max_idx] = max(full_polya_cnt_fit);
...
%plot(xbins,noise_polya_cnt_fit,'Linewidth',2);
noise_e_peak_amp.mean = sum(noise_polya_cnt_fit.*full_xbins)/sum(noise_polya_cnt_fit);
...
hold on;
h1=histogram(noise_mm_max_y(noise_glbl_cut),500,'DisplayName','Noise only');
hold on;
plot(full_xbins,full_polya_cnt_fit,'Linewidth',2);
hold on;
plot(full_xbins,noise_polya_cnt_fit,'Linewidth',2);
hold on;
plot(full_xbins,full_polya_cnt_fit-noise_polya_cnt_fit,'Linewidth',2);
legend('Full data','Noise only','Full data fit','Noise only fit','Full data fit - Noise only fit')
