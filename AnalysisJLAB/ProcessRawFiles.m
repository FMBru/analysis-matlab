...
%run.pathEOS=['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_April_h4\' run.oscilloscope '\Run' run.id '\'];
...
%tracker.path = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_April_h4\tracker\reconstructed\asciiRun' run.id '.dat'];
tracker.path = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_June_h4\trackerJLab\reconstructed\asciiRun' run.id '.dat'];
...
%run.debugPlotsPath=['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_April_h4\Results\DebugPlots\Run' run.id '-'  run.oscilloscope '\'];
run.debugPlotsPath=['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_June_h4\ResultsJLab\DebugPlots\Run' run.id '-'  run.oscilloscope '\'];
mkdir(run.debugPlotsPath);

%storeFolderSignalRef = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_April_h4\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\REF'];
storeFolderSignalRef = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_June_h4\ResultsJLab\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\REF'];
mkdir(storeFolderSignalRef);
%storeFolderSignalDUT = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_April_h4\Results\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\DUT'];
storeFolderSignalDUT = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_June_h4\ResultsJLab\Run' run.id '-' run.oscilloscope '-dut' opts_MM.chID '-ref' opts_MCP.chID '-' runInfoString '\signals\DUT'];
...
%(ElectronsetupMM), 4: MM4 (Picolarge main tracker), 5:MM5 (SiPM) August 2022

%Detector Mapping August 2023
%DUT index: 1:MM1, 2:MM2, 3:MM3, 4:MM4, 5:MM5, 6:MM6, 7:MCP2
%%dutColArray = [1 10 11; 2 13 14; 3 19 20; 4 22 23; 5 4 5; 6 25 26; 7 16 17]; %[dID colXID colYID; ] -> July 2023 MM run

%Detector Mapping June 2024 
%DUT index: 1:uRWELL-Pos1, 2:uRWELL-Pos2, 3:uRWELL-Pos3, 4:uRWELL-Pos4, 5:MCP-Pos
%dutColArray = [1 4 5; 2 7 8; 3 10 11; 4 13 14; 5 16 17; 6 19 20; 7 16 17]; %[dID colXID colYID; ] 

%mapping JLAB
%DUT index: 1:uRWELL-Pos1, 2:uRWELL-Pos2, 3:uRWELL-Pos3, 4:uRWELL-Pos4, 5:MCP-Pos5
dutColArray = [1 2 3; 2 5 6; 3 8 9; 4 11 12; 5 14 15]; %[dID colXID colYID; ] -> July 2023 MM run
...
for (ff=1:run.nfiles)
    ...
    if filesExists==1
        ...
        if tracker.en
                ...
                trackIdx = find(tracker.data(:,1) == eventIDTracker);
                if trackIdx>0
...
% 
%             outlierEventIDs = [2418 2870 2921 9053 10864 11097 11290 11309 12248 13232 15533 15863 21954 23314 26256 27176];
%             if ismember(eventIDTracker, outlierEventIDs) & shouldPlotOutliers
%                 shouldSave=1;
%             end


            if(minuit==1)
                MM_temp = process_signal_minuit(t_vec_mm-etime,ch_mm.y(:,m),opts_MM,run,shouldSave,storeFolderSignalDUT,eventIDTracker);
            else
            ...
            if(minuit==1)
                MCP_temp = process_signal_minuit(t_vec_mcp-etime,ch_mcp.y(:,m),opts_MCP,run,shouldSave,storeFolderSignalRef,eventIDTracker);
            else
