...
store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Results\Run' run.id '-' run.oscilloscope '\Combined'];
...
rmsCorrectionArray = zeros(length(activeChannels),4);
padPositionsArray = zeros(length(activeChannels),3);
...
for padCounter = 1:length(activeChannels)
    ...
    twCorrectionFile = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Results\Run' run.id '-' run.oscilloscope '\CH' num2str(channelID) '-Pad' num2str(padID) '\twCorrection.txt'];
    if exist(twCorrectionFile, 'file')
    ...
    rmsCorrectionArray(padCounter,1) = channelID;
    rmsCorrectionFile = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2024_September_h4\Results\Run' run.id '-' run.oscilloscope '\CH' num2str(channelID) '-Pad' num2str(padID) '\twCorrectionRMS.txt'];
    if exist(rmsCorrectionFile, 'file')
        correctionFactors = dlmread(rmsCorrectionFile);
        if length(correctionFactors)==3
            if isnan(correctionFactors(1)) || isnan(correctionFactors(2)) || isnan(correctionFactors(3)) || correctionFactors(1)==0 || correctionFactors(2)==0 || correctionFactors(3)==0
                %not valid correction factors
            else
                rmsCorrectionArray(padCounter,2) = correctionFactors(1);
                rmsCorrectionArray(padCounter,3) = correctionFactors(2);
                rmsCorrectionArray(padCounter,4) = correctionFactors(3);
            end
        end
    else
    end

    padPositionsArray(padCounter,1) = channelID;
    padPositionFile = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Results\Run' run.id '-' run.oscilloscope '\CH' num2str(channelID) '-Pad' num2str(padID) '\padPosition.txt'];
    if exist(padPositionFile, 'file')
        padPositions = dlmread(padPositionFile);
        if length(padPositions)==2
            if isnan(padPositions(1)) || isnan(padPositions(2)) || padPositions(1)==0 || padPositions(2)==0
                %not valid correction factors
            else
                padPositionsArray(padCounter,2) = padPositions(1);
                padPositionsArray(padCounter,3) = padPositions(2);
            end
        end
    else
    end
...
for eventPosCounter = 1:length(eventIDArrayUnique)
    ...
    e_peak_MMEvent = MM_epeak(eventIDMask);
    ...
    if numberDUTChannelsEvent>1
        ...
        meanTime = 0;
        meanTimeDiff=0;
        timeDiffWeightedSum=0;
        sumOfRMSWeightingFactors = 0;
        padPosCalculatedX = 0;
        padPosCalculatedY=0;
        ...
        for padPos = 1:numberDUTChannelsEvent
            ...
            %if dutChannelsEvent(padPos)==maxAmpPadNumber || isNeighboring(maxAmpPadNumber,dutChannelsEvent(padPos))
                ...
                %resolution for pad
                rmsEntryID = find(rmsCorrectionArray(:,1)==dutChannelsEvent(padPos));
                rmsCorrFactors = rmsCorrectionArray(rmsEntryID,2:4);
                if(twalk.en == 1) && rmsCorrFactors(1) ~= 0
                    padWeightingFactor = twalk_fn_minuit(rmsCorrFactors, e_peak_MMEvent(padPos));
                else 
                    padWeightingFactor = 1;
                end

                
                %position of pad
                positionEntryID = find(padPositionsArray(:,1)==dutChannelsEvent(padPos));
                positionEntry = padPositionsArray(positionEntryID,2:3);
                padPosX = positionEntry(1);
                padPosY = positionEntry(2);

                sumOfRMSWeightingFactors = sumOfRMSWeightingFactors+(1/(padWeightingFactor^2));
                timeDiffWeightedSum = timeDiffWeightedSum+padTimeDiff/(padWeightingFactor^2);
                % meanTime = meanTime+padTime*padWeight;
                %meanTimeDiff = meanTime-padTimeMCP;

                meanTimeDiff = meanTimeDiff+padTimeDiff*padWeight;
                padPosCalculatedX = padPosCalculatedX+padPosX*padWeight;
                padPosCalculatedY = padPosCalculatedY+padPosY*padWeight;

                includedPadIDPos = [includedPadIDPos;dutChannelsEvent(padPos)];
                includedPadIDAmp = [includedPadIDAmp;padWeight];
                
                if shouldPlotEvents
                str_disp=sprintf('maxAmpPadNumber: %d - Pad %d weight: %f - PosX: %f', maxAmpPadNumber,dutChannelsEvent(padPos), padWeight,padPosX);
                disp(str_disp);
                end
            end
        end
        numberDUTChannelsEvent;
        padPosCalculatedX;
%pause(5);
        timeDiffWeightedSum = timeDiffWeightedSum*sumOfRMSWeightingFactors;

        combinedEventsCombinedAmp (eventPosCounter) = totalAmp;
        combinedEventsCombinedTime (eventPosCounter) = meanTimeDiff;
        combinedEventsCombinedTimeRMSWeighted (eventPosCounter) = timeDiffWeightedSum;

        if trackerXEvent(1)~=0 && trackerYEvent(1) ~=0
            residualX = padPosCalculatedX-trackerXEvent(1);
            residualY = padPosCalculatedY-trackerYEvent(1);
    
            residualsX (eventPosCounter) = residualX;
            residualsY (eventPosCounter) = residualY;
    
            if shouldPlotEvents
                VisualiseMultipadEvent(includedPadIDPos,includedPadIDAmp,['Run ' run.id ' - Event ' num2str(eventID)],[store_folder '\Run' run.id '_event' num2str(eventID) '.png'],'Amp', '%f',0,0,trackerXEvent(1),trackerYEvent(1),padPosCalculatedX,padPosCalculatedY);
                pause(1);
                close all;
            end
        else 
            residualsX (eventPosCounter) = -100;
            residualsY (eventPosCounter) = -100;
        ...
        combinedEventsCombinedTimeRMSWeighted (eventPosCounter) = time_DiffSigmoidEvent;

        %position of pad
        positionEntryID = find(padPositionsArray(:,1)==dutChannelsEvent(1));
        positionEntry = padPositionsArray(positionEntryID,2:3);
        padPosX = positionEntry(1);
        padPosY = positionEntry(2);

        residualX = padPosX-trackerXEvent(1);
        residualY = padPosY-trackerYEvent(1);
...
% mask events without tracker
maskTrackerData = combinedEventsX~=0 & combinedEventsMultiplicty==1 & residualsX>-100 & residualsY>-100;
residualsXMasked = residualsX(maskTrackerData);
residualsYMasked = residualsY(maskTrackerData);
rmsXResiduals = std(residualsXMasked)
rmsYResiduals = std(residualsYMasked)

%% processed individual events, show overview plots

figure
hist(combinedEventsMultiplicty,1000)
...
for i=1:length(area.x_vec)
    for j=1:length(area.y_vec)
        ...
        area.combinedTimeRMSWeighted(i,j) = std(combinedEventsCombinedTimeRMSWeighted(cut_circ));
        area.residualsX(i,j) = std(residualsX(cut_circ));
        area.residualsY(i,j) = std(residualsY(cut_circ));
    end
end
...
h = colorbar;
caxis([1 4]);
h.Label.String = 'Multiplicity (# pads)';
...
h = colorbar;
h.Label.String = 'Combined amplitude V';
h.Label.Position(1) = 3;
...
%% plot Amp from neighboring pads over area
figure
...
str_title = sprintf('%s:\n Combined amplitude map (\\phi_{avg} = %2.1f mm)', runTitleString, 2*area.radius);
title(str_title);
saveas(gcf,[store_folder '\Run' run.id '_Amp_map.png'])




%% plot position residuals X
figure
h=pcolor(area.xx,area.yy,area.residualsX);
hold on
set(h, 'EdgeColor', 'none');
axis equal
colorbar
...
h = colorbar;
h.Label.String = 'Residuals X';
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n Residuals X (\\phi_{avg} = %2.1f mm)', runTitleString, 2*area.radius);
title(str_title);
...
h = colorbar;
h.Label.String = 'Combined time RMS (ps)';
h.Label.Position(1) = 3;
...
%% plot combined RMS from neighboring pads over area
figure
h=pcolor(area.xx,area.yy,area.combinedTimeRMSWeighted*1000);
hold on
set(h, 'EdgeColor', 'none');
axis equal
colorbar
...
h.Label.String = 'Combined time RMS (ps)';
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n Combined time RMS Weighted map (\\phi_{avg} = %2.1f mm)', runTitleString, 2*area.radius);
title(str_title);
saveas(gcf,[store_folder '\Run' run.id '_CombinedTimeRMSWeighted_map.png'])
...
%% plot combined RMS from neighboring pads over area
figure
h=pcolor(area.xx,area.yy,abs(area.combinedTime*1000));
hold on
set(h, 'EdgeColor', 'none');
axis equal
colorbar
...
h = colorbar;
h.Label.String = 'Combined SAT after correction by mean per pad (ps)';
h.Label.Position(1) = 3;
str_title = sprintf('%s:\n Combined SAT map (\\phi_{avg} = %2.1f mm)', runTitleString, 2*area.radius);
title(str_title);
saveas(gcf,[store_folder '\Run' run.id '_CombinedSAT_map.png'])
...
% e_peak_srt = sort(MM_epeak(glbl_cut));         % make temporary sort of e-peaks
...
%     temp_cut = MM_epeak > twalk.epeak_vec(i) & MM_epeak < twalk.epeak_vec(i+1) & glbl_cut;
%     twalk.mean_sat(i) = mean(time_diff_sigmoid(temp_cut));
%     twalk.e_peak(i) =  mean(MM_epeak(temp_cut));
...
%         pad.epeak_x(i) = mean(MM_epeak(tmp_cut));
%     end
%
%     fit_data = [];
...
%         pad.epeak_y(i) = mean(MM_epeak(tmp_cut));
%     end
%
%     fit_data = [];
...
%         area.e_peak(i,j) = mean(MM_epeak(cut_circ));
