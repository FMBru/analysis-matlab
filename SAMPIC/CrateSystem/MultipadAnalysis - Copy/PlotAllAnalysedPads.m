...
for resultPos = 1:length(padResults)
    ...
    if acceptPad
        padIDsArray = [padIDsArray;padResults(resultPos).padID];
        padCHArray = [padCHArray;getChannelForPadNumber(padResults(resultPos).padID,run.id)];
...
for resultPos = 1:length(padIDsArray)
    if padIDsArray(resultPos) == referencePadForStartingPoint
...
for pos = 1:length(padIDsArray)
    ...
    padIDsArray(pos);
    colMain = mod(padIDsArray(pos), 10);
    rowMain = (padIDsArray(pos) - colMain) / 10;
