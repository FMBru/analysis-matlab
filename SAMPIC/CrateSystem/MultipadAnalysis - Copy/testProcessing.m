                    ...
                    if length(matchedIdxToAnalyse)==1
                        ...
                        if size(trackerEntryMatched,1) %unambiguous match
                            ...
                            if trackerMinValue<1e15
                                ...
                                MM_temp = process_signal_sampic(t_vec_mm,dutWaveform(:,2),opts_MM,baselineDUT,run,shouldSave,storeFolderSignalDUT);
                                
                                % if signal is valid process MCP
                                if(MM_temp.fail==0)
                                    MCP_temp = process_signal_sampic(t_vec_mcp,refWaveform(:,2),opts_MCP,baselineMCP,run,shouldSave,storeFolderSignalRef);
                                    ...
                                    if(MCP_temp.fail==0)
                                        ...
                                        timeDiffSigmoidTemp = MM_temp.sigmoid.timepoint-MCP_temp.sigmoid.timepoint
                                        ...
                                        time_diff_sigmoid(eventPosProcessing) = timeDiffSigmoidTemp;
