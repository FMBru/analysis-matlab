...
addpath '\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Matlab\SAMPIC\CrateSystem\MultipadAnalysis\Functions'
padSide = 0.5*3.5;
...
pad1Pos = [94.0599 97.2475];  %July25 run 311 MM1 from alignment
%pad1Pos = [95 95]; %July25 run 311 MM1

padLength = 10;
padHeight = 10;

%set pad center positions
...
for runPos = 1:length(runIDsToAnalyse)
    ...
    if loadVariables
       ...
       % variablesFolder = ['C:\Users\GDD\Documents\Picosec\Run' run.id '-SAMPIC\variables'];
       % variablesFolder = ['F:\Picosec\Run' run.id '-SAMPIC\variables'];
    ...
    runTitleString = ['Multipad - July 25 - Run ' run.id];
    ...
    for c=1:10
        for r=1:10
            %                 padX = (r-1)*padLength+padLength/2 + pad1Pos(1);
            %                 padY = (c-1)*padHeight+padHeight/2 + pad1Pos(2);
    ...
    for eventPos = 1:1:length(uniqueEventIDs)
        ...
        if length(chIDArrayEvent)>0
            ...
            %trackerXEvent = -(trackerXEvent-pad1Pos(1))+pad1Pos(1);
            ...
            if max(MM_maxySum)>0.01 & numberAcceptedSignalsInEvent>=1 & trackerXEvent(1)>0 & trackerYEvent(1)>0
                ...
                for pos = 1:length(chIDArrayEvent)
                    ...
                    runID = run.id;
                    padIndex = getPadForChannelNumber(chNumber, runID);
                ...
                if shouldDisplayMaps & saveMapCounter<numberMapsToSave
                    VisualiseMultipadEvent(padCenters,padIDArrayEvent,1000*MM_maxyEvent,[runTitleString ' - Event ' num2str(eventTemp.eventID)],[store_folderEvents '\Event' int2str(eventTemp.eventID) '.png'],'','%0.1f mV',minC,maxC,trackerXEvent(1),trackerYEvent(1),positionRecoX,positionRecoY,run.id)
                    hold off;
                    pause(1);
                    close all;
    ...
    area.size = 15;      % set half-size of the observed square area, mm - for
    ...
    area.x_vec = area.x_vec + pad1Pos(1) - area.step/2;
    area.y_vec = area.y_vec + pad1Pos(2) - area.step/2;
