...
numberHitsForLatency = 200000000;
...
channelToAnalyse = [27;28;29;37;38;39];
...
previousFEBID = -1;
 ...
 if previousFEBID < 0
        ...
        while ~feof(fid) % && i<50000 %process full file
            if mod(i,10000)==0
                toc
                tic
            end
                                ...
                                if size(trackerEntryMatched,1) %unambiguous match
                                    ...
                                    if trackerMinValue<1e15
                                        ...
                                        etime=min([t_vec_mm; t_vec_mcp]);
                                                ...
                                                dutWithTracker.x = xPos;
                                                dutWithTracker.y = yPos;
                                                dutWithTracker.dut = dut;
                                                
                                                matchedChannelInclTracker = [matchedChannelInclTracker;dutWithTracker];
...
%                                                 currentChannel = event.matchedChannels(dutChannelPos);
                        ...
                        %output event with all matched channels incl.
                        %tracker
                        
                        numberMatchedInclTracker = length(matchedChannelInclTracker);
                        
                        if numberMatchedInclTracker>0
                            refWaveform = ref.waveform;
                            xPos = matchedChannelInclTracker(1).x;
                            yPos = matchedChannelInclTracker(1).y;
                            
                            sampicMatchedDataFilepath = 'testOutput.csv';
                            
                            headerLine = [xPos yPos numberMatchedInclTracker];
                            
                            dlmwrite(sampicMatchedDataFilepath,headerLine,'-append')
                            
                            refWaveformOutput = refWaveform.';
                            dlmwrite(sampicMatchedDataFilepath,refWaveformOutput,'-append', 'precision', 16)
                            
                            for matchedEventPos = 1:numberMatchedInclTracker
                                dutWaveform = matchedChannelInclTracker(matchedEventPos).dut.waveform;
                                dutWaveformOutput = dutWaveform.';
                                dlmwrite(sampicMatchedDataFilepath,matchedChannelInclTracker(matchedEventPos).dut.ch,'-append')
                                dlmwrite(sampicMatchedDataFilepath,dutWaveformOutput,'-append', 'precision', 16)
                            end
                        end
