function out = process_signal_sampic(x,y,options,baseline,run,shouldSave,store_folder)
...
%     end
    ...
    while(sigmoid.time20 < 0.2*sig.max.y & sigmoid.sevals<5e4)
    ...
    % first go forward
    while(sigmoid.time10 < 0.1*sig.max.y & sigmoid.sevals10<5e4)
    ...
    % first go forward
    while(sigmoid.time90 < 0.9*sig.max.y & sigmoid.sevals90<5e4)
