...
analysis.dutChannel = 108; %comment out for analysing multiple files with AnalyseAllChannels.m
...
addpath 'C:\Users\GDD\Documents\MATLAB\Picosec\SAMPIC\CrateSystem'
...
dataFileLocal='C:\Users\GDD\Documents\Picosec\October22\MatlabRunData';
...
store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2023_July_h4\Results\Run' run.id '-' run.oscilloscope '\CH' num2str(analysis.dutChannel) '-Pad' num2str(getPadForChannelNumber(analysis.dutChannel))];
mkdir(store_folder);
...
MM_epeakDUT = MM_epeak(channelMask);
MCP_epeakDUT = MCP_epeak(channelMask);
MM_riseTimeDUT = MM_riseTime(channelMask);
MM_bgAvgDUT = MM_bgAvg(channelMask);
MM_bgRMSDUT = MM_bgRMS(channelMask);
MM_sampicTOTDUT = MM_sampicTOT(channelMask);
MM_sampicAmpDUT = MM_sampicAmp(channelMask);
%% extract from structurearrays

k=1;

% clear  e_peak_MM e_peak_MCP riseTime
% 
...
%     MM_epeakDUT(i)=MM_dataDUT(i).sig.charge.e_peak;
%     MCP_epeakDUT(i)=MCP_dataDUT(i).sig.charge.e_peak;% extract electrom peak charge
%     MM_riseTimeDUT(i)=MM_dataDUT(i).sigmoid.timepoint90-MM_dataDUT(i).sigmoid.timepoint10;% extract electrom peak charge
% end

if length(MCP_epeakDUT)>0
...
% find events within the cut and cut with respect to amplirudes and
   ...
   % glbl_cut =  trackerXDUT~=0 & trackerYDUT~=0;
    ...
    if length(time_diff_cut)>10
        ...
        e_peak_srt = sort(MM_epeakDUT(glbl_cut));         % make temporary sort of e-peaks
        ...
        if length(twalk.epeak_vec)>0
            for i=1:length(twalk.epeak_vec)-1
                temp_cut = MM_epeakDUT > twalk.epeak_vec(i) & MM_epeakDUT < twalk.epeak_vec(i+1) & glbl_cut;
                temp_cut = MM_epeakDUT > twalk.epeak_vec(i) & MM_epeakDUT < twalk.epeak_vec(i+1) & glbl_cut;
                twalk.mean_sat(i) = mean(time_diff_sigmoidDUT(temp_cut));
                twalk.e_peak(i) =  mean(MM_epeakDUT(temp_cut));
            ...
            if(twalk.en == 1)
                time_diff_sigmoidDUT = time_diff_sigmoidDUT - twalk_fn_minuit(twalk.p, MM_epeakDUT);
            end
        end
    end
    ...
    if minimalPlots==false
        ...
        if alignToDUT == true
            ...
            for i = 1:length(pad.h_x.Values)
                ...
                pad.epeak_x(i) = mean(MM_epeakDUT(tmp_cut));
            end
            ...
            for i = 1:length(pad.h_y.Values)
                ...
                pad.epeak_y(i) = mean(MM_epeakDUT(tmp_cut));
            end
        ...
        if alignToDUT == false
            ...
            for i = 1:length(pad.h_x.Values)
                tmp_cut = trackerXDUT>pad.h_x.BinEdges(i) & trackerXDUT<pad.h_x.BinEdges(i)+ pad.h_x.BinWidth;
                pad.epeak_x_mcp(i) = mean(MCP_epeakDUT(tmp_cut));
            end
            ...
            for i = 1:length(pad.h_y.Values)
                tmp_cut = trackerYDUT>pad.h_y.BinEdges(i) & trackerYDUT<pad.h_y.BinEdges(i)+pad.h_y.BinWidth;
                pad.epeak_y_mcp(i) = mean(MCP_epeakDUT(tmp_cut));
            end
        ...
        if length(glbl_cutTrue)>10 && min(trackerXDUT(glbl_cut))~=max(trackerXDUT(glbl_cut)) && min(trackerYDUT(glbl_cut))~=max(trackerYDUT(glbl_cut))
            ...
            for i=1:length(area.x_vec)
                for j=1:length(area.y_vec)
                    ...
                    area.amp(i,j) = mean(MM_maxyDUT(cut_circ));
                    area.amp_MCP(i,j) = mean(MCP_maxyDUT(cut_circ));
                    area.e_peak(i,j) = mean(MM_epeakDUT(cut_circ));
...
%             ylim([-10 110]);
            ...
            figure
            h=histogram(MM_riseTimeDUT(glbl_cut),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            ...
            xlim([0 3]);
            %legend('MM','MM fit');
            title_str = sprintf('%s \n Signal rise time \\mu = %4.4f ns',runTitleString, mean(MM_riseTimeDUT(glbl_cut)));
            title(title_str)
            saveas(gcf,[store_folder '\Run' run.id '_riseTimeHist.png'])
            
            
            %% plot bg level distribution
            figure
            h=histogram(1000*MM_bgAvgDUT(glbl_cut),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            ...
            xlim([-10 10]);
            %legend('MM','MM fit');
            title_str = sprintf('%s \n Baseline level \\mu = %4.4f mV',runTitleString, 1000*mean(MM_bgAvgDUT(glbl_cut)));
            title(title_str)
            saveas(gcf,[store_folder '\Run' run.id '_bgAvg.png'])
            
            
            
             %% plot bg RMS distribution
            figure
            h=histogram(1000*MM_bgRMSDUT(glbl_cut),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            ...
            xlim([0 5]);
            %legend('MM','MM fit');
            title_str = sprintf('%s \n Baseline RMS \\mu = %4.4f mV',runTitleString, 1000*mean(MM_bgRMSDUT(glbl_cut)));
            title(title_str)
            saveas(gcf,[store_folder '\Run' run.id '_bgRMS.png'])
           
            
            %% plot electron charge
%             for i=1:length(MM_epeakDUT)
%                 MM_e_charge(i)=MM_dataDUT(i).sig.charge.e_peak;
%             end
            figure;
            plot(MM_maxyDUT(glbl_cut), MM_epeakDUT(glbl_cut),'.');
            grid on
            xlabel('Signal amplitude, V')
...
%             end
            figure;
            h=histogram(MM_epeakDUT(glbl_cut),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            fit_data = [];
            fit_data(1,:)=1:length(xbins);
  ...
  %% plot peak amplitude histogram cut included - Only MM
            figure
            h=histogram(MM_sampicAmpDUT(cut_sampling),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            fit_data = [];
            fit_data(1,:)=1:length(xbins);
            ...
            xlabel('Signal amplitude SAMPIC, V')
            ylabel('events');
            grid on
            legend('MM','MM fit');
            title_str = sprintf('%s \n e-peak amplitude SAMPIC \\mu = %4.4f V U_{max} = %4.4f V',runTitleString, mean(MM_sampicAmpDUT(cut_sampling)));
            title(title_str)
            saveas(gcf,[store_folder '\Run' run.id '_signalAmplitude_SAMPIC_MM-samplingArea.png'])


  %% plot TOT histogram cut included - Only MM
            figure
            h=histogram(MM_sampicTOTDUT(cut_sampling),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            fit_data = [];
            fit_data(1,:)=1:length(xbins);
            ...
            xlabel('TOT SAMPIC, ns')
            ylabel('events');
            grid on
            legend('MM','MM fit');
            title_str = sprintf('%s \n e-peak amplitude SAMPIC \\mu = %4.4f ns U_{max} = %4.4f V',runTitleString, mean(MM_sampicTOTDUT(cut_sampling)));
            title(title_str)
            saveas(gcf,[store_folder '\Run' run.id '_TOT_SAMPIC_MM-samplingArea.png'])
...
%             end
            figure;
            plot(MM_maxyDUT(cut_sampling), MM_epeakDUT(cut_sampling),'.');
            grid on
            xlabel('Signal amplitude, V')
            ylabel('Electron charge, pC');
            title_str = sprintf('%s \n Electron charge plot',runTitleString);
...
%             end
            figure;
            h=histogram(MM_epeakDUT(cut_sampling),100);
            hold on
            xbins = h.BinEdges(1:end-1)+h.BinWidth/2;
            fit_data = [];
            fit_data(1,:)=1:length(xbins);
            ...
            for j=1:length(sym_angles)
                ...
                for i = 1:line_amp.n
                    ...
                    line_amp.avg(i) = mean(MM_maxyDUT(cut_circ));
                    line_amp.n_hits(i) = sum(cut_circ);
                    line_amp.err(i) = std(MM_maxyDUT(cut_circ))./sqrt(line_amp.n_hits(i));
                end
...
%                     MM_dataDUT(k).sig.max.y, MCP_dataDUT(k).sig.charge.e_peak, MM_dataDUT(k).sig.charge.e_peak, trackerXDUT(k),trackerYDUT(k));
            ...
            bgAvg = mean(MM_bgAvgDUT(cut_sampling));
            bgRMS = mean(MM_bgRMSDUT(cut_sampling));
            padRiseTime = mean(MM_riseTimeDUT(cut_sampling));
            padTWCorrectionEpeak = twalk.e_peak;
            padTWCorrectionSAT = twalk.mean_sat;
            padTWCorrection = twalk.p;
