...
clear MM_maxyDUT
        ...
        padIDsArray = [1,2,3,4,5,6,7];
          channelToAnalyse = [1,2,3,4,5,6,7];
        for i=1:length(padIDsArray)
            channelToAnalyse (i) = getChannelForPadNumberPicolarge(padIDsArray(i));
        end
...
VisualisePicolarge(padCHArray,padCountsArray,'Hitmap Multipad Picosec',[store_folder '\Run' run.id '_Hitmap.png'],'Hits','%0.1f %',0,0);

VisualisePicolarge(padCHArray,padMaxYArray,'Mean Amplitude Multipad Picosec',[store_folder '\Run' run.id '_MeanAmpMap.png'],'Mean','%0.1f mV',0,0);

VisualisePicolarge(padCHArray,padMaxYFitArray,'Mean Amplitude from Fit Multipad Picosec',[store_folder '\Run' run.id '_MeanAmpFromFitMap.png'],'Mean','%0.1f mV',0,0);
