...
for runPos=1:length(runIDsToAnalyse)
    ...
    %storeMatfileFolder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\SAMPIC\Processing\Run' run.id '-SAMPIC\variables'];
    ...
    if length(dataFilesList)>0
        ...
        VisualiseMediumGranularity(channelsEnabled,chCountsArray,'Hitmap Multipad Picosec Relative',[store_folder '\Run' run.id '_HitmapRelative.png'],'Hits','%0.1f %',0,0,run.id);
        VisualiseMediumGranularity(channelsEnabled,chCountsArrayNumbers,'Hitmap Multipad Picosec',[store_folder '\Run' run.id '_Hitmap.png'],'Hits','%0.1f %',0,6000,run.id);
        ...
        VisualiseMediumGranularity(channelsEnabled,abs(chLatencyArray),'Latency Multipad Picosec',[store_folder '\Run' run.id '_Latency.png'],'Latency','%0.1f ns',0,0,run.id);
        ...
        %%save ref signals
        numberVars = floor(length(signalsRef)/100000);
        for k = 1:numberVars
