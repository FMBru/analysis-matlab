...
%analysis.dutChannel = 24; %comment out for analysing multiple files with AnalyseAllChannels.m
...
runTitleString = [run.name ' ' run.oscilloscope ' DUT:ch' num2str(analysis.dutChannel) ' - Pad ' num2str(getPadForChannelNumber(analysis.dutChannel,run.id))];
...
store_folder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Results\Run' run.id '-' run.oscilloscope '\CH' num2str(analysis.dutChannel) '-Pad' num2str(getPadForChannelNumber(analysis.dutChannel,run.id))];
mkdir(store_folder);

channelMask = padIDArray==analysis.dutChannel;
   ...
   % glbl_cut =  trackerXDUT~=0 & trackerYDUT~=0;
    ...
    if length(time_diff_cut)>10
        ...
        if length(twalk.epeak_vec)>0
            ...
            % fit correction function using minuit - TW parameters used for
            % combining signals
            fit_data = [];
            ...
            % fit resolution as function of charge amp
            fit_data = [];
            fit_data(1,:) = twalk.e_peak;
            fit_data(2,:) = twalk.rms;
            fit_data(3,:) = twalk.err;
            p0=[];
            p0(1) = min(twalk.rms);
            p0(2) = 1;
            p0(3) = 0.5;
            cmd='min; ret';
            [pRMS, errRMS, chiRMS] = fminuit('twalk_fn_minuit',p0,fit_data,'-b','-c',cmd);
            twalk.pRMS = pRMS;
            twalk.chiRMS = chiRMS;
            ...
            hold on
            plot(twalk.e_peak,twalk_fn_minuit(twalk.pRMS,twalk.e_peak)*1000,'LineWidth',1.5);
            xlabel('Electron peak charge, pC')
    ...
    dlmwrite([store_folder '\twCorrectionRMS.txt'],twalk.pRMS)
...
%                     MM_dataDUT(k).sig.max.y, MCP_dataDUT(k).sig.charge.e_peak, MM_dataDUT(k).sig.charge.e_peak, trackerXDUT(k),trackerYDUT(k));
            ...
            if exist('twalk.e_peak') 

            else
                twalk.e_peak = 0;
            end

             if exist('twalk.mean_sat') 
            ...
            numberEntriesAnalysedFullPad = length(time_diff_sigmoidDUT);
            numberEntriesAnalysedFullPadGlblCut = length(time_diff_cut);
...
%             bgRMS = mean(MM_bgRMSDUT(cut_sampling));
            ...
            outputData = [pad.xc pad.yc];
            dlmwrite([store_folder '\padPosition.txt'],outputData)
