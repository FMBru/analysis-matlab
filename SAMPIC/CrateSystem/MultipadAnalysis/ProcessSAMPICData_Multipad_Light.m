clearvars -except runIDsToAnalyse channelsEnabled channelToAnalyse
tic
...
%light for check
% hitsPerSegment = 20000; %how many hits to save per segment
% numberHitsForLatency = 50000;
% processingSegmentSize = 10000;

hitsPerSegment = 100000; %how many hits to save per segment
hitsPerRefSegment = 100000; %how many ref hits to save per segment
processingSegmentSize = 10000;
numberHitsForLatency = 100000;

eventsLimit = 30000000; %max events to process (same for ref and signals)
...
%channelToAnalyse = [0 12 13 14 22 23 24 32 33 34];

%channelToAnalyse = [0 23 24 33 34];

numberFEBoards = 2;
numberFEBoards = 1; %only some channels on first card
...
for runPos=1:length(runIDsToAnalyse)
    ...
    storeMatfileFolder = ['\\eosproject-smb\eos\project\p\picosec\testbeam\2025_July_h4\Results\Run' run.id '-SAMPIC\variables'];
    storeMatfileFolder = ['F:\Picosec\Run' run.id '-SAMPIC\variables'];
    ...
    if length(dataFilesList)>0
        ...
        VisualiseMultipad(channelsEnabled,chCountsArray,'Hitmap Multipad Picosec Relative',[store_folder '\Run' run.id '_HitmapRelative.png'],'Hits','%0.1f %',0,0,run.id);
        VisualiseMultipad(channelsEnabled,chCountsArrayNumbers,'Hitmap Multipad Picosec',[store_folder '\Run' run.id '_Hitmap.png'],'Hits','%0.1f %',0,6000,run.id);
        
        %%determine latency for channels relative to reference detector
        disp('Determining latency');
        refChannelMask = chIDsArray==referenceDetectorChannel;
        refTimesArray = chTimesArray(refChannelMask);
        ...
        for chPos = 1:length(channelsEnabled)
            if  channelsEnabled(chPos) == referenceDetectorChannel
                ...
                [refTimeCounts,refTimeCenters] = hist(refTimesArray,10000);
                ...
                if length(shouldAnalyseChannel)==1
                    ...
                    for k = 1:length(signalsDUTchannel)
                        ...
                        timeDiffToRef = detectorHitTime-refTimesArray;
                        minTimeDiffToRef = min(abs(timeDiffToRef));
                        if minTimeDiffToRef<200 && minTimeDiffToRef~0 && signalAmplitude>0.01;
                            timeDiffToRefArray = [timeDiffToRefArray;minTimeDiffToRef];
                        end
                    end
        ...
        storeMatfilePath = [store_folderLatency '\latencies.mat'];
        m = matfile(storeMatfilePath,'Writable',true);
        save(storeMatfilePath,'channelsEnabled');
        save(storeMatfilePath,'chLatencyArray','-append');
        
        %plot latency accross multipad
        VisualiseMultipad(channelsEnabled,abs(chLatencyArray),'Latency Multipad Picosec',[store_folder '\Run' run.id '_Latency.png'],'Latency','%0.1f ns',0,0,run.id);
        ...
        str_disp=sprintf('Reading in Binary data file for ref signal extraction');
        disp(str_disp);
        
        dataFilesList;
        
        
        %% loop through files to extract segment of signals
        signalsRef = [];
        chIDsRefArray = [];
        chTimesRefArray = [];
        chIndexRefArray = [];
        ...
        for filePos=1:length(dataFilesList)
            ...
            while ~feof(fid) & eventPos<eventsLimit %process full file
                if mod(i,10000)==0
                    toc
                    tic
                end
                ...
                % count overflows of timestamp
                timestampRaw = Cell0TimeStamp;
                if(timestampRaw < (timestamp_prev-1e12))
                    timestampRaw;
                    timestamp_prev;
                    
                    timeStampDiff = timestampRaw-timestamp_prev;
                ...
                if length(channel)>0
                    ...
                    if channel == referenceDetectorChannel
                        ...
                        if dataSize>10
                            ...
                            if length(signalsRef)==0
                                ...
                                chIndexRefArray = [1];
                                    ...
                                    signalsRef (eventPos) = hit;
                                    chIDsRefArray (eventPos) = channel;
                                    chTimesRefArray (eventPos) = timestamp;
                                    chIndexRefArray (eventPos) = length(signalsRef);
                            end
                            eventPos = eventPos+1;
                        end
                        ...
                        if length(shouldAnalyseChannel)==1
                            if dataSize>10
                                ...
                                if length(signals)==0
                                    ...
                                    chIDsArray = [channel];
                                    chTimesArray = [timestamp];
                                    chIndexArray = [eventPosSignal];
                                else
                                    signals (eventPosSignal) = hit;
                                    chIDsArray (eventPosSignal) = channel;
                                    chTimesArray (eventPosSignal) = timestamp;
                                    chIndexArray (eventPosSignal) = eventPosSignal;
                                end
                                
                                if chTimesArray (eventPosSignal) ~= signals (eventPosSignal).cell0Time
                                    chTimesArray (eventPosSignal)
                                    pause(5);
                                end


                                eventPosSignal = eventPosSignal+1;
                            end
                        ...
                        if length(signals) > hitsPerSegment
                            ...
                            %save signals
                            n_strPadded = sprintf( '%03d', numberVarsSignal ) ;
                            storeMatfilePath = [storeMatfileFolder '\signals_' n_strPadded '.mat'];
                            m = matfile(storeMatfilePath,'Writable',true);
                            ...
                            numberVarsSignal = numberVarsSignal+1;
                        end

                        if length(signalsRef) > hitsPerSegment
                            %save ref signals array
                            
                            signalsRefSegment = signalsRef;
                            chIDsRefArraySegment = chIDsRefArray;
                            chTimesRefArraySegment = chTimesRefArray;
                            chIndexRefArraySegment = chIndexRefArray;
                            %save signals
                            n_strPadded = sprintf( '%03d', numberVarsRef ) ;
                            storeMatfilePath = [storeMatfileFolder '\signalsRef_' n_strPadded '.mat'];
                            mkdir(storeMatfileFolder)
                            m = matfile(storeMatfilePath,'Writable',true);
                            %
                            save(storeMatfilePath,'signalsRefSegment');
                            save(storeMatfilePath,'chIDsRefArraySegment','-append');
                            save(storeMatfilePath,'chTimesRefArraySegment','-append');
                            save(storeMatfilePath,'chIndexRefArraySegment','-append');
...
%         %save ref signals
%         numberVars = ceil(length(signalsRef)/hitsPerRefSegment);
%         for k = 1:numberVars
%             startIndex = hitsPerRefSegment*(k-1)+1;
%             endIndex = k*hitsPerRefSegment;
%             if endIndex>length(signalsRef)
%                 endIndex = length(signalsRef);
%             end
...
%         
%         str_disp=sprintf('Reading signals from files...');
%         disp(str_disp);
%         
%         for filePos=1:length(dataFilesList)
%             
...
%                             chIDsArray = [channel];
%                             chTimesArray = [timestamp];
%                             chIndexArray = [eventPos];
%                         else
%                             signals (eventPos) = hit;
%                             chIDsArray (eventPos) = channel;
%                             chTimesArray (eventPos) = timestamp;
%                             chIndexArray (eventPos) = eventPos;
%                         end
%                         
%                         
%                         eventPos = eventPos+1;
...
%                     
%                     %save signals
%                     n_strPadded = sprintf( '%03d', numberVars ) ;
%                     storeMatfilePath = [storeMatfileFolder '\signals_' n_strPadded '.mat'];
%                     m = matfile(storeMatfilePath,'Writable',true);
%                     save(storeMatfilePath,'signals');
%                     save(storeMatfilePath,'chIDsArray','-append');
