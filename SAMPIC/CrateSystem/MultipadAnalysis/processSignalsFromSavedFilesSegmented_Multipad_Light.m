...
%runIDsToAnalyse = ["218"];
channelsEnabled = [0 12 13 14 22 23 24 32 33 34];
%channelsEnabled = [0 23 24 33 34];
...
for runPos = 1:length(runIDsToAnalyse)
    ...
    variablesFolder = storeMatfileFolder;
...
scaledEventCounterTimes = eventCounterEntries(:,2);
    ...
    disp(str_disp);
    variablesFilesListAll = dir(variablesFolder);
    
        names = {variablesFilesListAll.name};         % Extract filenames
        [~, idx] = sort(names);       % Get sorting order
        variablesFilesListAll = variablesFilesListAll(idx);           % Reorder struct array
        names = {variablesFilesListAll.name};                     % Extract names into cell array
        mask  = contains(names, 'signalsRef');           
        variablesFilesList = variablesFilesListAll(mask);           % Reorder struct array
    ...
    for i=1:length(variablesFilesList)
        ...
        if contains(fileInfoRef.name,'.mat') & contains(fileInfoRef.name,'signalsRef')
            ...
            minRefTime = min(segmentRef.chTimesRefArraySegment);
            maxRefTime = max(segmentRef.chTimesRefArraySegment);
            
            refTimeLimitsArray = [refTimeLimitsArray;[refFileIndex minRefTime maxRefTime]];
    ...
    for i=1:length(variablesFilesList)
        ...
        str_disp=sprintf('Loading ref file %d / %d', i, numberFiles);
        disp(str_disp);
        
        if contains(fileInfoRef.name,'.mat') & contains(fileInfoRef.name,'signalsRef_')
            str_disp=sprintf('Processing ref signals file');
            disp(str_disp);
            filePathRef = [variablesFolder '\' fileInfoRef.name];
            segmentRef = load(filePathRef);
            ...
            str_disp=sprintf(['Loading DUT files for ref file ' num2str(i)]);
            disp(str_disp);
            
            names = {variablesFilesListAll.name};                     % Extract names into cell array
            mask  = contains(names, 'signals_');           
            variablesFilesListSignals = variablesFilesListAll(mask);           % Reorder struct array
            variablesFilesListSignals = variablesFilesListSignals(~[variablesFilesListSignals.isdir]);      
            
            numberDUTFilesLoaded = 0;
            for dutFilePos=1:length(variablesFilesListSignals)
                fileInfo = variablesFilesListSignals(dutFilePos);
                ...
                if contains(fileInfo.name,'.mat') & contains(fileInfo.name,'signals_')
                    ...
                    if minTimeSegment>=minRefTime & minTimeSegment<=maxRefTime %starting in this ref segment
                        %load DUT file
                        disp(['DUT file ' num2str(dutFilePos) ' in ref time range']);
                ...
                for chDUTPos = 1:length(channelsEnabled)
                    if  channelsEnabled(chDUTPos) ~= referenceDetectorChannel
                        ...
                        if length(shouldAnalyseChannel)==1
                            ...
                            %timeDiffArray = zeros(length(chTimesArrayDUT),1);
                            
                            %for timeDiffPos = 1:length(timeDiffArray)
                            %    timeDiffArray(timeDiffPos) = signalsArrayDUT(timeDiffPos).cell0Time - refSignal.cell0Time;
                            %end
                            ...
                            if length(chIndexMatched)==1 %unambiguous hit
                                ...
                                signalsArrayDUT = signalsSegment(dutChannelMask);
                                
                                %time between signals
                                correctTime = timeDiffArray(matchMask);
                                signalTemp = signalsArrayDUT(matchMask);

                                timesTestArray = [timesTestArray;[correctTime (signalTemp.cell0Time-refSignal.cell0Time)]];
